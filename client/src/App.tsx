  ...
  return <>
    <header className={`fixed top-0 z-50 w-full transition-all duration-300 ${scrolled || lightRoute ? "border-b border-[#123c43]/10 bg-[#f6f2ea]/90 text-[#123c43] backdrop-blur-xl" : "bg-transparent text-white"}`}>
      <div className="mx-auto flex h-20 max-w-[1440px] items-center justify-between px-6 lg:px-12">
        <Link href="/" className="flex items-center gap-3" aria-label="SVARA home"><Mark /><span className="font-display text-2xl tracking-[0.16em]">SVARA</span></Link>
        <nav className="hidden items-center gap-7 lg:flex">{links.map(([label, href], i) => <Link key={href} href={href} className={`relative py-2 text-[10px] font-semibold uppercase tracking-[0.22em] transition-opacity hover:opacity-60 ${location === href ? "opacity-100" : "opacity-70"}`}>{label}{location === href && <span className="absolute -bottom-1 left-0 h-px w-full bg-current" />}</Link>)}<Link href="/contact" className={`ml-3 flex items-center gap-2 border px-4 py-2.5 text-[10px] font-bold uppercase tracking-[0.2em] transition-all hover:bg-[#c97861] hover:text-white hover:border-[#c97861] ${scrolled ? "border-[#123c43]" : "border-white/60"}`}>Book a visit <ArrowUpRight size={13} /></Link></nav>
